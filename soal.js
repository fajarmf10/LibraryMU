var save_method, table, editor_id;
...
$(function(){
   ...
   table = $('.table').DataTable({
      "processing" : true,
      "ajax" : {
         "url" : "ajax/ajax_soal.php?action=table_data&quiz="+quiz,
         "type" : "POST"
      }
   });
...
function save_data(){
   quiz = $('#quiz').val();
   if(save_method == "add")
      url = "ajax/ajax_soal.php?action=insert&quiz="+quiz;
...
function import_data(){
   ...
   quiz = $('#id_quiz').val();
   $.ajax({
      url: 'ajax/ajax_soal.php?action=import&quiz='+quiz,
      data: formdata,
      processData: false,
      contentType: false,
      type: 'POST',
